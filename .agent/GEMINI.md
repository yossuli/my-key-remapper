...
- **Props Grouping (Late Destructuring)**:
  ...
  - **目的（可読性）**: 末端のコンポーネント定義の冒頭で分割代入を行うことで、そのコンポーネントが「具体的にどのデータ/関数を使用しているか」を一目で把握できるようにするため。
  - これにより、中間層の Props 定義を簡潔に保ちつつ、末端の実装の詳細（依存関係）を明確にする。
...
- **[Workflow-009] プロップスフロー可視化によるリファクタリングの推進**: 伝播をマトリクスで可視化し、Prop Drilling を発見・解消してコンポーネントの自律性を高める手順。 (`.agent/workflows/009.md`)
