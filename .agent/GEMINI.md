...
### 影響範囲管理（Impact Assessment）

- **広範囲な変更（High Impact）**を行う可能性がある場合（例：全ファイルへの適用、基盤コンポーネントの変更）：
  - 実装に入る前に必ず**計画（Implementation Plan）**を提示し、承認を得てください。
  - `Workflow-0`（認識合わせ）の段階で影響範囲を判定し、必要であれば計画策定フェーズを挟んでください。
