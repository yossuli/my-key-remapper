...
- グローバルルールは必ず守ってください
- **ログ（log.md）の永続的蓄積（絶対命令）**:
  - `log.md` の上書き、過去ログの削除、または断片化（リセット）は、**プロジェクトの破壊行為**とみなします。
  - 常に **「最新を一番上に追記（Append-Only）」** し、作業開始からの全履歴を一貫して保持し続けてください。
  - どのような状況でも、過去の経緯を消してはいけません。
- **ログによる自律規律と自己修正（絶対命令）**:
  - `log.md` の **Planned セクションは「ワークフロー探索の起点」** です。作業前に必ず適用するワークフローを明記してください。
  - **Accomplished セクションは「自己検閲の場」** です。Planned と対比させ、ワークフローを適用し忘れたことに気づいた場合は、**直ちに作業を中断・破棄し、ワークフローを適用した形で正しくやり直して** ください。
